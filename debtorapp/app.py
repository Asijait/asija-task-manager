...
def create_database_checkpoint(note=""):
    """
    Creates a PostgreSQL custom-format database checkpoint.

    The checkpoint is created BEFORE any database-changing import.

    Returns:
        str: Absolute path to the backup file.

    Raises:
        RuntimeError: If the PostgreSQL backup cannot be created.
    """

    database_url = os.environ.get("DEBTOR_DATABASE_URL") or os.environ.get(
        "DATABASE_URL"
    )
    ...
    backup_path = os.path.join(
        BACKUP_DIR,
        f"db_checkpoint_{timestamp}.dump",
    )
    ...
    try:
        ...
        raise RuntimeError(
            "pg_dump is not installed on the production server."
        ) from exc

    except subprocess.TimeoutExpired as exc:
        raise RuntimeError("PostgreSQL checkpoint timed out.") from exc
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            "PostgreSQL checkpoint failed: "
            + (result.stderr.strip() or "Unknown pg_dump error.")
        )

    if not os.path.isfile(backup_path):
        raise RuntimeError("pg_dump completed but no backup file was created.")

    if os.path.getsize(backup_path) <= 0:
        raise RuntimeError("PostgreSQL checkpoint is empty.")

    add_backup_log_entry(
        {
            "id": timestamp,
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "type": "database-checkpoint",
            "note": note or "Database checkpoint",
...
DEBTOR_NAV_ACCESS_ITEMS = [
    {
        "id": "import_tally_debtors",
        "label": "Import Tally Debtors",
        "group": "Import / Export",
        "view_key": "",
        "full_key": "import_tally_debtors",
    },
    {
        "id": "import_tally_receipts",
        "label": "Import Tally Receipts",
        "group": "Import / Export",
        "view_key": "import_tally_receipts_view",
        "full_key": "import_tally_receipts",
    },
    {
        "id": "download_report_excel",
        "label": "Reports in Excel Format",
        "group": "Import / Export",
        "view_key": "download_report_excel",
        "full_key": "download_report_excel",
    },
    {
        "id": "client_master",
        "label": "Client Master",
        "group": "Masters",
        "view_key": "client_master_view",
        "full_key": "client_master_edit",
    },
    {
        "id": "client_group_master",
        "label": "Client Group Master",
        "group": "Masters",
        "view_key": "client_group_master_view",
        "full_key": "client_group_master_edit",
    },
    {
        "id": "crp_master",
        "label": "CRP Master",
        "group": "Masters",
        "view_key": "crp_master_view",
        "full_key": "crp_master_edit",
    },
    {
        "id": "firm_master",
        "label": "Firm Master",
        "group": "Masters",
        "view_key": "firm_master_view",
        "full_key": "firm_master_edit",
    },
    {
        "id": "executive_partner_master",
        "label": "Executive Partner",
        "group": "Masters",
        "view_key": "executive_partner_master_view",
        "full_key": "executive_partner_master_edit",
    },
    {
        "id": "run_followup_logic",
        "label": "Run Followup Logic",
        "group": "Masters",
        "view_key": "",
        "full_key": "run_followup_logic",
    },
    {
        "id": "clear_report",
        "label": "Clear Report",
...
def insert_deleted_payload(cursor, table_name, payload):
    ...
    if "id" in columns and payload.get("id") not in (None, ""):
        cursor.execute(
            f"SELECT id FROM {table_name} WHERE id = ?", (payload.get("id"),)
        )
        if cursor.fetchone():
...
def restore_deleted_log_record(cursor, delete_log_id):
    ...
    source_table = (
        log_row["source_table"] if isinstance(log_row, sqlite3.Row) else log_row[1]
    )
    payload_json = (
        log_row["payload_json"] if isinstance(log_row, sqlite3.Row) else log_row[6]
    )
    payload = json.loads(payload_json or "{}")

    if source_table == "billing_report":
        row_id = payload.get("id") or (
            log_row["source_pk"] if isinstance(log_row, sqlite3.Row) else log_row[2]
        )
        duplicate_ref = find_active_billing_ref(
            cursor, payload.get("ref_no"), exclude_id=row_id
        )
        if duplicate_ref:
            raise ValueError(
...
def build_deleted_records_rows(cursor):
        ...
        rows.append(
            {
                ...
                "record_type": row_dict.get("display_type")
                or row_dict.get("source_table")
                or "",
                "record_name": row_dict.get("display_label")
                or row_dict.get("source_pk")
                or "",
        ...
        rows.append(
            {
                ...
                "record_name": row_dict.get("party_name")
    ...
    rows.sort(
        key=lambda item: (
            ...
            str(item.get("delete_log_id") or item.get("legacy_billing_id") or ""),
        ),
        reverse=True,
    )
    return rows
...
def update_normalized_references(cursor, table_name, column_name, old_name, new_name):
    ...
    for row in cursor.fetchall():
        row_dict = sqlite_record_to_dict(row)
        if (
            master_name_key(row_dict.get(column_name)) == old_key
            and row_dict.get(column_name) != new_name
        ):
            cursor.execute(
...
def dedupe_master_table(
    cursor, table_name, name_column, merge_columns=None, reference_columns=None
):
    ...
    for rows in grouped_rows.values():
        ...
        for duplicate in rows[1:]:
            ...
            for ref_table, ref_column in reference_columns:
                update_normalized_references(
                    cursor, ref_table, ref_column, duplicate_name, canonical_name
                )

            for column_name in merge_columns:
                if not normalize_master_name(
                    update_values.get(column_name, primary.get(column_name))
                ):
                    update_values[column_name] = duplicate.get(column_name)

            cursor.execute(
                f"DELETE FROM {table_name} WHERE id = ?", (duplicate.get("id"),)
            )
            removed_count += 1
        ...
        if set_parts:
            ...
            cursor.execute(
                f"UPDATE {table_name} SET {', '.join(set_parts)} WHERE id = ?", params
            )
...
def soft_delete_billing_rows(cursor, row_ids, delete_reason=""):
    ...
    for row_id in row_ids:
        cursor.execute(
            "SELECT * FROM billing_report WHERE id = ? AND deleted_at IS NULL",
            (row_id,),
        )
        row = cursor.fetchone()
        if not row:
...
def ensure_client_group_master_seed(cursor):
    ...
    for row in cursor.fetchall():
        group_name = (
            row["client_group"] if isinstance(row, sqlite3.Row) else row[0]
        ) or ""
        crp_name = (
            row["crp_of_group"] if isinstance(row, sqlite3.Row) else row[1]
        ) or ""
        reffered_by = (
            row["reffered_by"] if isinstance(row, sqlite3.Row) else row[2]
        ) or ""
        ...
        if not group_name or group_key in seeded_groups:
            continue
        existing_group = find_master_by_name(
            cursor, "client_group_master", "group_name", group_name
        )
        if existing_group:
            cursor.execute(
                """
            ...
            """,
                (
                    group_name,
                    normalize_master_name(crp_name),
                    normalize_master_name(reffered_by),
                ),
            )
...
def ensure_crp_master_seed(cursor):
    ...
    """)
    crp_names.extend(
        row["crp_name"] if isinstance(row, sqlite3.Row) else row[0]
        for row in cursor.fetchall()
    )
    cursor.execute("""
        SELECT crp_of_group
    ...
    """)
    crp_names.extend(
        row["crp_of_group"] if isinstance(row, sqlite3.Row) else row[0]
        for row in cursor.fetchall()
    )
...
def sync_client_master_from_group_master(cursor):
    ...
    for row in cursor.fetchall():
        group_name = (
            row["group_name"] if isinstance(row, sqlite3.Row) else row[0]
        ) or ""
...
def get_all_debtor_nav_access():
    try:
        ...
        conn.commit()
        rows = cursor.execute(
            "SELECT user_email, access_key FROM debtor_nav_access"
        ).fetchall()
        conn.close()
    except sqlite3.Error:
        return {}
...
def get_debtor_access_key_for_path(path):
    ...
    if (
        ...
        in (
            ...
            "/bulk-import",
            "/bulk-import/confirm",
            "/import-followup-choice",
        )
        or path == "/download/billing-template"
    ):
        return "import_tally_debtors"
    if path == "/import-errors" or path == "/download/missing-clients-report":
        return "import_tally_debtors"
    if (
        path in ("/import-tally-receipts", "/tally-receipts/import-summary")
        or path == "/api/import-tally-receipts/preview"
    ):
        return "import_tally_receipts_view"
    if (
        path == "/import-tally-receipts/upload"
        or path == "/api/import-tally-receipts/post"
    ):
    ...
    if path in (
        "/client-master/add",
        "/client-master/update",
        "/client-master/upload",
        "/client-master/group/add",
        "/download/client-template",
    ):
        return "client_master_edit"
    if (
        path == "/client-group-master"
        or path == "/download/client-group-master-excel"
        or (
            path.startswith("/client-group-master/")
            and (
                path.endswith("/clients")
                or path.endswith("/clients-data")
                or path.endswith("/clients-excel")
            )
        )
    ):
        return "client_group_master_view"
    if path in (
        "/client-group-master/add",
        "/client-group-master/update",
        "/client-group-master/delete",
    ):
        return "client_group_master_edit"
    if (
        path == "/crp-master"
        or path == "/download/crp-master-excel"
        or (
            path.startswith("/crp-master/")
            and (path.endswith("/groups-data") or path.endswith("/groups-excel"))
        )
    ):
...
def get_manual_report_update_dates():
    ...
    try:
        ...
        defaults["last_bill_update_date"] = get_app_meta(
            cursor, "manual_last_bill_update_date"
        )
        defaults["last_receipt_update_date"] = get_app_meta(
            cursor, "manual_last_receipt_update_date"
        )
        conn.close()
    except sqlite3.Error:
        pass
...
def get_report_as_on_label():
    ...
    try:
        ...
        cursor.execute(
            "SELECT MAX(receipt_date) AS latest_receipt_date FROM receipt_register"
        )
    ...
    if latest_bill_date:
        labels.append(
            f"Bill Report As On (Bill Import): {format_header_date(latest_bill_date)}"
        )
    ...
    if receipt_label_date:
        ...
        labels.append(
            f"Receipt Last {label_type}: {format_header_date(receipt_label_date)}"
        )
...
def create_postgres_backup(backup_dir, backup_id):
    """
    Create a PostgreSQL custom-format dump.

    Returns:
        str: Absolute path to the generated dump file.

    Raises:
        RuntimeError: If PostgreSQL is configured but pg_dump fails.
    """
    database_url = os.environ.get("DEBTOR_DATABASE_URL") or os.environ.get(
        "DATABASE_URL"
    )
    ...
    if not database_url:
        raise RuntimeError(
            "PostgreSQL backup requested but "
            "DEBTOR_DATABASE_URL / DATABASE_URL is not configured."
        )

    os.makedirs(backup_dir, exist_ok=True)

    dump_name = f"postgres_{backup_id}.dump"
    ...
    try:
        ...
        raise RuntimeError(
            "pg_dump was not found on the server. Install PostgreSQL client tools."
        ) from exc
    ...
    if os.path.getsize(dump_path) == 0:
        raise RuntimeError("PostgreSQL backup file is empty.")
...
def create_project_backup(note="", backup_type="manual"):
    ...
    try:
        # ======================================================
        # 1. CREATE POSTGRESQL SNAPSHOT FIRST
        # ======================================================

        if sqlite3.use_postgres(DB_PATH):
            postgres_dump_path = create_postgres_backup(BACKUP_DIR, stamp)
        ...
        with zipfile.ZipFile(temp_zip_path, "w", zipfile.ZIP_DEFLATED) as backup_zip:
            ...
            if postgres_dump_path:
                backup_zip.write(
                    postgres_dump_path, arcname=os.path.basename(postgres_dump_path)
                )

                backup_zip.writestr(
                    "DATABASE_BACKUP_INFO.txt",
                    (
            ...
            for current_dir, dirs, files in os.walk(BASE_DIR):
                ...
                for filename in files:
                    ...
                    # Never put the PostgreSQL dump into
                    # itself recursively.
                    if postgres_dump_path and os.path.abspath(
                        file_path
                    ) == os.path.abspath(postgres_dump_path):
                        continue

                    # Don't backup the backup directory.
                    if os.path.abspath(file_path).startswith(
                        os.path.abspath(BACKUP_DIR) + os.sep
                    ):
                        continue

                    # Don't treat the PostgreSQL database
                    # as a physical SQLite file.
        ...
        # The dump is already inside the ZIP.
        # Remove the standalone dump.
        if postgres_dump_path and os.path.exists(postgres_dump_path):
            try:
    ...
    add_backup_log_entry(
        {
            ...
            "file": zip_name,
            "database": ("postgresql" if sqlite3.use_postgres(DB_PATH) else "sqlite"),
            "skipped": skipped,
        }
    )
...
def restore_postgres_backup(dump_path):
    """
    Restore a PostgreSQL custom-format dump.

    IMPORTANT:
    This replaces the database contents.
    """
    database_url = os.environ.get("DEBTOR_DATABASE_URL") or os.environ.get(
        "DATABASE_URL"
    )
    ...
    if not database_url:
        raise RuntimeError(
            "PostgreSQL restore requested but "
            "DEBTOR_DATABASE_URL / DATABASE_URL "
            "is not configured."
        )

    if not os.path.exists(dump_path):
        raise RuntimeError("PostgreSQL dump file does not exist.")
    ...
    try:
        ...
        raise RuntimeError("pg_restore was not found on the server.") from exc
    ...
    if result.returncode != 0:
        raise RuntimeError(
            ...
            + (result.stderr.strip() or "Unknown pg_restore error.")
        )
...
def restore_project_backup(backup_id):
    ...
    if sqlite3.use_postgres(DB_PATH):
        try:
            ...
            # before restoring the older checkpoint.
            current_backup_id, current_skipped = create_project_backup(
                note=("Automatic safety backup before PostgreSQL rollback"),
                backup_type="pre-rollback",
            )

            app.logger.info("Created pre-rollback backup: %s", current_backup_id)
            ...
            with tempfile.TemporaryDirectory(dir=BACKUP_DIR) as temp_dir:
                with zipfile.ZipFile(zip_path, "r") as backup_zip:
                    dump_members = [
                        ...
                        if (name.startswith("postgres_") and name.endswith(".dump"))
                    ]

                    if not dump_members:
                        return (
                            False,
                            "This backup does not contain a PostgreSQL database dump.",
                        )

                    dump_name = dump_members[0]

                    backup_zip.extract(dump_name, temp_dir)

                    dump_path = os.path.join(temp_dir, dump_name)

                restore_postgres_backup(dump_path)
            ...
            add_backup_log_entry(
                {
                    ...
                    "created_at": (datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
                    "type": "rollback",
                    "note": (f"PostgreSQL restored from checkpoint {backup_id}"),
            ...
            app.logger.exception("PostgreSQL rollback failed")
...
def format_financial_year(value):
    ...
    start_year = (
        bill_date.year
        if (bill_date.month, bill_date.day) >= (4, 2)
...
def parse_tally_receipt_trial(search_date="", file_path=None, file_name=None):
    ...
    receipt_name = file_name or (
        os.path.basename(receipt_path)
        if isinstance(receipt_path, (str, bytes, os.PathLike))
        else "fcRece.xlsx"
    )
    if isinstance(receipt_path, (str, bytes, os.PathLike)) and not os.path.exists(
        receipt_path
    ):
    ...
    for candidate in workbook.worksheets:
        ...
        for index, row in enumerate(candidate_rows):
            if any(
                str(cell).strip().lower() == "date" for cell in row if cell is not None
            ):
    ...
    for zero_index, row in enumerate(rows[data_start_index:], start=data_start_index):
        ...
        if row_date:
            current_voucher = {
                ...
                "debit_amount": float(cells[8] or 0)
            ...
            current_ledger = {
                ...
                "credit_amount": float(cells[9] or 0)
                if str(cells[9] or "").strip()
                else 0,
        ...
        if label in {"agst ref", "new ref"}:
            receipt_item = {
                ...
                "reference_amount": float(cells[4] or 0)
                if str(cells[4] or "").strip()
                else 0,
                "reference_dr_cr": normalize_tally_cell(cells[5]),
                "bank_name": current_voucher["bank_name"],
                "raw_rows": list((current_ledger or {}).get("raw_rows", []))
                + [zero_index + 1],
            }
...
def tally_posted_key(voucher_no, reference_no, receipt_date, amount):
    ...
    normalized_date = (
        date_obj.strftime("%Y-%m-%d") if date_obj else str(receipt_date or "").strip()
    )
    normalized_amount = f"{round(float(amount or 0), 2):.2f}"
    return (
        voucher_no or "",
        normalize_reference_key(reference_no),
        normalized_date,
        normalized_amount,
    )
...
def ensure_receipt_register_tally_columns(cursor):
    ...
    for column_name, column_type in required_columns.items():
        if column_name not in existing_columns:
            cursor.execute(
                f"ALTER TABLE receipt_register ADD COLUMN {column_name} {column_type}"
            )
...
def enrich_tally_receipt_rows(tally_rows, cursor):
    ...
    for row in tally_rows:
        ...
        posted_key = tally_posted_key(
            voucher_no, item.get("reference_no"), item.get("receipt_date"), amount
        )
        ...
        if posted_key in seen_preview_keys:
            ...
            item["post_message"] = (
                "This voucher/reference appears more than once in the preview."
            )
        elif posted_key in posted_keys:
            item["post_status"] = "Already Posted"
            item["post_message"] = (
                "This Tally voucher/reference is already in receipt register."
            )
            ...
            if posted_receipts:
                ...
                posted_total = round(
                    sum(
                        float(receipt["received_amount"] or 0)
                ...
                posted_dates = sorted(
                    {
                        format_display_date(receipt["receipt_date"])
                        for receipt in posted_receipts
                        if receipt["receipt_date"]
                    }
                )
                item["matched_ref_no"] = (
                    posted_receipt["ref_no"]
                    or posted_receipt["tally_reference_no"]
                    or ""
                )
            ...
            if amount <= 0:
                ...
                item["post_message"] = (
                    "Ready to post. Receipt amount is greater than current bill balance; bill will be closed."
                )
            ...
            item["post_status"] = "Already Posted"
            item["post_message"] = (
                "This reference number is not open in main report and is already available in receipt register."
            )
            item["matched_ref_no"] = (
                posted_receipt["ref_no"] or posted_receipt["tally_reference_no"] or ""
            )
            item["matched_party_name"] = posted_receipt["party_name"] or ""
            item["matched_bill_amount"] = posted_total
            item["posted_receipt_count"] = len(posted_receipts)
...
def post_receipt_rows(
    cursor,
    receipt_rows,
    receipt_mode,
    receipt_date,
    posted_at,
    import_meta_by_bill=None,
):
    ...
    for item in receipt_rows:
        ...
        if not row:
            raise ValueError(
                "One selected bill is no longer available in the debtor report."
            )
...
def post_adjustment_rows(
    cursor, adjustment_rows, adjustment_type, adjustment_date, posted_at
):
    current_rows = {int(row["id"]): row for row in get_report_rows(cursor)}
...
def run_followup_partner_logic(cursor):
    ...
    for row in rows:
        ...
        row["final_ep"] = (
            row.get("ep_override") or ""
        ).strip() or infer_final_ep_from_ref(row.get("ref_no"), ep_lookup)
    ...
    for group_name, data in group_data.items():
        ...
        if not ep_names:
            ...
            followup_by_group[group_name] = max(
                non_crp_ep_totals, key=non_crp_ep_totals.get
            )
        else:
            followup_by_group[group_name] = crp_name
...
def resolve_followup_partner_for_row(cursor, billing_row_id):
    ...
    target_row = next(
        (row for row in rows if int(row.get("id") or 0) == int(billing_row_id)), None
    )
    if not target_row:
        return ""

    target_group = (
        target_row.get("client_group") or target_row.get("party_name") or ""
    ).strip()
    if not target_group:
        return ""
    ...
    group_rows = [
        row
        for row in rows
        if (row.get("client_group") or row.get("party_name") or "").strip()
        == target_group
    ]
    crp_name = ""
    ep_totals = {}
    for row in group_rows:
        ...
        ep_name = (row.get("final_ep") or "").strip()
        if ep_name:
            ep_totals[ep_name] = ep_totals.get(ep_name, 0) + float(
                row.get("amount") or 0
            )

    ep_names = set(ep_totals)
    non_crp_ep_totals = {
        ep_name: total for ep_name, total in ep_totals.items() if ep_name != crp_name
    }
...
def get_existing_followup_partner_for_row(cursor, billing_row_id):
    ...
    target_group = (
        ...
        if isinstance(target_row, sqlite3.Row)
        else target_row[0]
    ) or ""
...
def resolve_or_copy_followup_partner_for_row(cursor, billing_row_id):
    return get_existing_followup_partner_for_row(
        cursor, billing_row_id
...
def ensure_cheque_bounce_register_table(cursor):
    ...
    for bounce_row in cursor.fetchall():
        ...
        cursor.execute(
            "SELECT * FROM billing_report WHERE id = ?", (duplicate_bill_id,)
        )
        ...
        source_balance = (
            0
            if source.get("receipt_status") == "full_paid"
            else float(source.get("amount") or 0)
        )
        duplicate_balance = (
            0
            if duplicate.get("receipt_status") == "full_paid"
            else float(duplicate.get("amount") or 0)
        )
...
def readd_receipts_as_cheque_bounce(cursor, receipt_ids, reason="", bounce_date=""):
    ...
    for row in receipt_rows:
        ...
        if not due_date:
            ...
            due_date = (
                (bill_date_obj + timedelta(days=30)).strftime("%Y-%m-%d")
                if bill_date_obj
                else ""
            )
        ...
        cursor.execute(
            "SELECT * FROM billing_report WHERE id = ? LIMIT 1", (source_bill_id,)
        )
        ...
        current_balance = (
            0
            if source_bill.get("receipt_status") == "full_paid"
            else float(source_bill.get("amount") or 0)
        )
        restored_balance = round(current_balance + bounced_amount, 2)
        restored_paid = max(
            0, round(float(source_bill.get("paid_amount") or 0) - bounced_amount, 2)
        )
        cursor.execute(
            """
            UPDATE billing_report
        ...
        followup_partner = source_bill.get(
            "followup_partner"
...
def copy_existing_followup_for_import_batch(cursor, batch_id):
    ...
    for row in cursor.fetchall():
        ...
        followup_partner = (
            row["followup_partner"] if isinstance(row, sqlite3.Row) else row[1]
        ) or ""
    ...
    for row in rows:
        ...
        group_name = (
            (row["group_name"] if isinstance(row, sqlite3.Row) else row[1]) or ""
        ).strip()
        followup_partner = followup_by_group.get(group_name)
...
def find_report_header(df):
    for row_index in range(min(len(df), 30)):
        ...
        next_labels = (
            [normalize_cell(value) for value in df.iloc[row_index + 1].tolist()]
        ...
        date_idx = next(
            (index for index, label in enumerate(combined) if label == "date"), None
        )
        ref_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if label in ("refno", "refnumber", "referenceno")
            ),
            None,
        )
        party_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if "party" in label and "name" in label
            ),
            None,
        )
        amount_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if label in ("amount", "pendingamount")
                or ("pending" in label and "amount" in label)
            ),
            None,
        )
        due_idx = next(
        ...
        firm_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if label in ("firm", "firmname", "company", "companyname")
            ),
            None,
        )
        if (
            date_idx is not None
            and ref_idx is not None
            and party_idx is not None
...
def find_sales_register_header(df, source_filename=""):
    ...
    title_is_sales_register = (
        "salesregister" in title_labels or "salesregister" in "".join(title_labels)
    )
    ...
    for row_index in range(min(len(df), 30)):
        ...
        party_idx = next(
            (index for index, label in enumerate(combined) if label == "particulars"),
            None,
        )
        ref_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if label in ("vchno", "voucherno", "vouchernumber")
            ),
            None,
        )
        debit_idx = next(
        ...
        amount_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if label in ("amount", "netamount", "invoiceamount")
            ),
            None,
        )
        voucher_type_idx = next(
            (
                index
                for index, label in enumerate(combined)
                if label in ("vchtype", "vouchertype")
            ),
            None,
        )
        firm_idx = next(
        ...
        if (
            ...
            and party_idx is not None
            and ref_idx is not None
            and (debit_idx is not None or amount_idx is not None)
        ):
...
def extract_import_rows(data_file):
    ...
    def add_row(
        ...
        if not bill_date_obj or not ref_text or not party_text or amount is None:
            return
        due_date_obj = (
            parse_input_date(due_date_value) if due_date_value is not None else None
        )
    ...
    with pd.ExcelFile(data_file) as xls:
        ...
        if header:
            for row_index in range(header["row_index"] + 1, len(df)):
                ...
                if header.get("firm_idx") is not None and not is_blank(
                    row.iloc[header["firm_idx"]]
                ):
                    row_firm = str(row.iloc[header["firm_idx"]]).strip()
                due_value = (
                    row.iloc[header["due_idx"]]
                    if header.get("due_idx") is not None
                    else None
                )
                add_row(
                    row_firm,
                    row.iloc[header["date_idx"]],
                    row.iloc[header["ref_idx"]],
                    row.iloc[header["party_idx"]],
                    row.iloc[header["amount_idx"]],
                    due_value,
                )
            return extracted_rows
            ...
            for row_index in range(sales_header["row_index"] + 1, len(df)):
                ...
                if voucher_type_idx is not None:
                    ...
                    if voucher_type and "sale" not in voucher_type:
                        continue
                row_firm = fallback_firm
                if sales_header.get("firm_idx") is not None and not is_blank(
                    row.iloc[sales_header["firm_idx"]]
                ):
                    row_firm = str(row.iloc[sales_header["firm_idx"]]).strip()
                add_row(
                    row_firm,
                    row.iloc[sales_header["date_idx"]],
                    row.iloc[sales_header["ref_idx"]],
                    row.iloc[sales_header["party_idx"]],
                    row.iloc[sales_header["amount_idx"]],
                )
            return extracted_rows
...
def process_data_file(data_file, manual_firm_name="", source_filename=""):
    ...
    try:
        create_database_checkpoint(
            note=(
                f"Before debtor import: "
                f"{source_filename or os.path.basename(data_file)}"
            )
        )
    except Exception as exc:
        print(f"Database Checkpoint failed.Import Aborted: {exc}")
    ...
    try:
        ...
        def note_duplicate_ref(ref_no):
            ...
            if (
                sample_ref
                and sample_ref not in duplicate_ref_samples
                and len(duplicate_ref_samples) < 10
            ):
        ...
        if data_file.endswith(".csv"):
            with open(data_file, "r", encoding="utf-8", errors="ignore") as f:
                ...
                firm_name = (
                    manual_firm_name.strip() or first_nonblank(reader[0]) or firm_name
                )
                rows_data = reader[1:]
            ...
            with pd.ExcelFile(data_file) as xls:
                ...
                if not df_firm.empty:
                    firm_name = (
                        manual_firm_name.strip()
                        or first_nonblank(df_firm.iloc[0].tolist())
                        or firm_name
                    )
                ...
                if header:
                    ...
                    for row_index in range(start_index, len(df)):
                        row = df.iloc[row_index]
                        try:
                            bill_date_obj = parse_input_date(
                                row.iloc[header["date_idx"]]
                            )
                            ...
                            due_date_obj = (
                                parse_input_date(row.iloc[header["due_idx"]])
                                if header["due_idx"] is not None
                                else None
                            )
                            row_firm_name = ""
                            ...
                            effective_firm_name = (
                                manual_firm_name.strip() or row_firm_name or firm_name
                            )
                            if insert_billing_row(
                                cursor,
                                effective_firm_name,
                    ...
                    if sales_header and sales_header["is_sales_register"]:
                        ...
                        for row_index in range(start_index, len(df)):
                            ...
                            try:
                                voucher_type_idx = sales_header.get("voucher_type_idx")
                                if voucher_type_idx is not None:
                                    voucher_type = normalize_cell(
                                        row.iloc[voucher_type_idx]
                                    )
                                    if voucher_type and "sale" not in voucher_type:
                                        continue
                                bill_date_obj = parse_input_date(
                        ...
                        for row in df_main.values.tolist():
                            try:
                                ...
                                continue
        missing_client_count = save_import_client_errors(
            cursor, batch_id, imported_rows
        )
        if imported_count > 0:
            set_app_meta(
                cursor, "last_debtor_imported_at", datetime.now().strftime("%Y-%m-%d")
            )
        conn.commit()
        return {
...
def init_db():
    ...
    for src, dst_dir in [
        (os.path.join(BASE_DIR, "style.css"), STATIC_DIR),
    ...
    for column_name, column_type in required_client_columns.items():
        if column_name not in existing_client_columns:
            cursor.execute(
                f"ALTER TABLE client_master ADD COLUMN {column_name} {column_type}"
            )
    ...
    for column_name, column_type in required_group_columns.items():
        if column_name not in existing_group_columns:
            cursor.execute(
                f"ALTER TABLE client_group_master ADD COLUMN {column_name} {column_type}"
            )
    ...
    if "receipt_status" not in existing_billing_columns:
        cursor.execute(
            "ALTER TABLE billing_report ADD COLUMN receipt_status TEXT DEFAULT 'open'"
        )
    if "paid_amount" not in existing_billing_columns:
        cursor.execute(
            "ALTER TABLE billing_report ADD COLUMN paid_amount REAL DEFAULT 0"
        )
...
def build_report_page_context(active_page="report", page_title="Firm Billing Report"):
    ...
    for row in rows:
        ...
        for receipt in receipts:
            ...
            bill_amount = (
                float(receipt["bill_amount"] or 0)
                if "bill_amount" in receipt.keys()
                else 0
            )
            balance_amount = (
                float(receipt["balance_amount"] or 0)
                if "balance_amount" in receipt.keys()
                else 0
            )
        ...
        total_received = sum(
            float(receipt["received_amount"] or 0) for receipt in receipts
        )
...
def report():
    try:
        ...
        if "database is locked" in str(exc).lower():
            flash(
                "Debtor report database is busy/locked. Please try again after the database lock is cleared.",
                "error",
            )
        else:
...
def build_dashboard_context():
    ...
    high_risk_rows = sorted(
        rows, key=lambda row: float(row.get("amount") or 0), reverse=True
    )[:8]
        ...
        overdue_rows.append(
            {
                ...
                "bill_date": row.get("bill_date") or "",
                "bill_date_display": row.get("bill_date_display")
                or format_display_date(row.get("bill_date")),
                "ref_no": row.get("ref_no") or "",
                "party_name": row.get("party_name") or "",
                "amount": float(row.get("amount") or 0),
                "amount_display": format_indian_currency(
                    row.get("amount") or 0, decimals=False
                ),
                "due_date": row.get("due_date") or "",
                "due_date_display": row.get("due_date_display")
...
def current_overdue_report_excel():
    ...
    selected_rows = [
        rows_by_id[row_id] for row_id in cleaned_ids if row_id in rows_by_id
    ]
...
def receipt_register():
    ...
    return render_template(
        "receipt_register.html", rows=rows, active_page="receipt_register"
    )
...
def mark_receipts_cheque_bounce():
    ...
    bounce_date = (
        request.form.get("bounce_date") or datetime.now().strftime("%Y-%m-%d")
    ).strip()
    if not receipt_ids:
    ...
    try:
        cursor = conn.cursor()
        result = readd_receipts_as_cheque_bounce(
            cursor, receipt_ids, reason, bounce_date
        )
        conn.commit()
...
def delete_receipt_register_record():
    ...
    try:
        ...
        cursor.execute(
            "DELETE FROM receipt_register WHERE id = ?", (receipt_dict.get("id"),)
        )
        conn.commit()
...
def adjustment_register():
    ...
    return render_template(
        "adjustment_register.html", rows=rows, active_page="adjustment_register"
    )
...
def delete_adjustment_register_record():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM receipt_adjustment_register WHERE id = ?", (adjustment_id,)
        )
        ...
        cursor.execute(
            "DELETE FROM receipt_adjustment_register WHERE id = ?",
            (adjustment_dict.get("id"),),
        )
        conn.commit()
...
def receipt_register_weekly_report():
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(
            sorted(summary.values(), key=lambda item: item["Followup Partner"])
        ).to_excel(
            writer,
            index=False,
        ...
        pd.DataFrame(detail_rows).to_excel(
            writer, index=False, sheet_name="Receipt Details"
        )
...
def build_weekly_recovery_context(
    start_date_obj, end_date_obj, cumulative_start_obj=None, partner=""
):
    start_date = start_date_obj.strftime("%Y-%m-%d")
    end_date = end_date_obj.strftime("%Y-%m-%d")
    if start_date > end_date:
    ...
    for row in rows:
        ...
        row["bill_amount_display"] = format_indian_currency(
            row.get("bill_amount") or 0, decimals=False
        )
        row["received_amount_display"] = format_indian_currency(
            received_amount, decimals=False
        )
        row["balance_amount_display"] = format_indian_currency(
            row.get("balance_amount") or 0, decimals=False
        )
        item = partner_summary.setdefault(
            partner,
            {
                "partner": partner,
                "receipt_count": 0,
    ...
    summary_rows = sorted(
        partner_summary.values(), key=lambda item: item["received_amount"], reverse=True
    )
    ...
    for row in summary_rows:
        row["received_amount_display"] = format_indian_currency(
            row["received_amount"], decimals=False
        )
    ...
    return {
        ...
        "cumulative_total_display": format_indian_currency(
            cumulative_total, decimals=False
        ),
...
def weekly_recovery_report():
    start_date_obj = parse_input_date(request.args.get("start_date", "").strip())
    end_date_obj = parse_input_date(request.args.get("end_date", "").strip())
    cumulative_start_obj = parse_input_date(
        request.args.get("cumulative_start_date", "").strip()
    )
    ...
    return render_template(
        ...
        **build_weekly_recovery_context(
            start_date_obj, end_date_obj, cumulative_start_obj, partner
        ),
    )
...
def build_tally_receipt_import_summary(start_date="", end_date=""):
    ...
    for row in detail_rows:
        row["import_date_display"] = format_header_date(
...
def add_client_master_dropdowns(writer, sheet_name, dropdown_options, max_rows=1000):
    ...
    for option_index, (header, values) in enumerate(dropdown_options.items(), start=1):
        ...
        option_column_letter = options_sheet.cell(
            row=1, column=option_index
        ).column_letter
        ...
        validation.error = (
            "Please select a value from master dropdown or type a valid new value."
        )
...
def import_tally_receipts_upload():
    ...
    if ext not in {".xlsx", ".xlsm"}:
        flash(
            "Invalid receipt file. Please upload fcRece.xlsx or another .xlsx receipt export.",
            "error",
        )
        return redirect(url_for("import_tally_receipts"))
    ...
    try:
        ...
        preview = parse_tally_receipt_trial(
            "", file_path=io.BytesIO(file_bytes), file_name=file.filename
        )
        if not preview.get("data_start_row") or not preview.get("rows"):
            flash(
                "Receipt file was not imported because Date header/receipt rows were not found. Old fcRece.xlsx is unchanged.",
                "error",
            )
            return redirect(url_for("import_tally_receipts"))
        ...
        flash(
            f"Receipt file was not imported: {exc}. Old fcRece.xlsx is unchanged.",
            "error",
        )
        return redirect(url_for("import_tally_receipts"))

    flash(
        f"Receipt file imported as {os.path.basename(TALLY_RECEIPT_TRIAL_PATH)}.",
        "success",
    )
...
def import_tally_receipts_preview():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Unable to preview Tally receipts: {exc}"}
        ), 500
...
def import_tally_receipts_post():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Unable to read Tally receipts: {exc}"}
        ), 500
    ...
    try:
        create_database_checkpoint(
            note=(
                f"Before Tally Receipt Import"
                f"{f' - {search_date}' if search_date else ''}"
            )
        )
    except Exception as exc:
        return jsonify(
            {
                "success": False,
                "message": (
                    f"Receipt import aborted.Database checkpoint failed: {exc}"
                ),
            }
        ), 500
    ...
    try:
        ...
        for row in postable_rows:
            ...
            if (
                receipt_date_obj
                and row_receipt_date
                and row_receipt_date.strftime("%Y-%m-%d")
                != receipt_date_obj.strftime("%Y-%m-%d")
            ):
                continue
            receipt_date = (
                row_receipt_date or receipt_date_obj or datetime.now()
            ).strftime("%Y-%m-%d")
            ...
            import_meta_by_date.setdefault(receipt_date, {})[bill_id] = {
                ...
                "tally_excel_rows": ", ".join(
                    str(value) for value in (row.get("raw_rows") or [])
                ),
            }
        ...
        set_app_meta(
            cursor, "last_receipt_activity_at", max(receipt_rows_by_date.keys())
        )
        ...
        conn.close()
        return jsonify(
            {"success": False, "message": f"Unable to post Tally receipts: {exc}"}
        ), 500
...
def post_receipts():
    ...
    if receipt_mode not in {
        ...
        return jsonify(
            {"success": False, "message": "Please select a valid receipt mode."}
        ), 400
    if not receipt_date_obj:
        return jsonify(
            {"success": False, "message": "Please select a valid date."}
        ), 400
    if not isinstance(receipt_rows, list) or not receipt_rows:
        return jsonify(
            {"success": False, "message": "Please select at least one bill."}
        ), 400
    ...
    try:
        if is_adjustment:
            post_adjustment_rows(
                cursor, receipt_rows, receipt_mode, receipt_date, posted_at
            )
        ...
        conn.close()
        return jsonify(
            {"success": False, "message": f"Unable to post receipt: {exc}"}
        ), 500
...
def sub_report():
    ...
    followup_summary = dict(
        sorted(followup_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    )
    ep_summary = dict(
        sorted(ep_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    )
    category_summary = dict(
        sorted(category_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    )

    all_fys = sorted(
        list(set(row.get("financial_year") or "Unknown" for row in rows)), reverse=True
    )
...
def get_sub_report_detail_rows(partner="", ep="", category="", fy="", ageing=""):
    ...
    rows = get_report_rows(cursor)
    conn.close()
    return filter_sub_report_rows(
        rows, partner=partner, ep=ep, category=category, fy=fy, ageing=ageing
    )
...
def render_sub_report_detail(partner="", ep="", category="", fy="", ageing=""):
    ...
    filtered_rows = get_sub_report_detail_rows(
        partner=partner, ep=ep, category=category, fy=fy, ageing=ageing
    )
    total_amount = sum(int(round(row.get("amount") or 0)) for row in filtered_rows)
    title = build_sub_report_detail_title(
        partner=partner, ep=ep, category=category, fy=fy, ageing=ageing
    )
...
def sub_report_detail_excel():
    ...
    fy = (request.args.get("fy") or "").strip()
    ageing = (request.args.get("ageing") or "").strip()
    rows = get_sub_report_detail_rows(
        partner=partner, ep=ep, category=category, fy=fy, ageing=ageing
    )
    title = build_sub_report_detail_title(
        partner=partner, ep=ep, category=category, fy=fy, ageing=ageing
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(export_rows).to_excel(
            writer, index=False, sheet_name="Detailed Report"
        )
...
def periodic_report():
    ...
    for row in rows:
        ...
        amt = int(round(row.get("amount") or 0))
        g_name = (
            row.get("client_group") or row.get("party_name") or "Unassigned"
        ).strip()
        ...
        if g_name not in group_analysis:
            group_analysis[g_name] = {
                "total": 0,
                "fys": set(),
                "first_date": bill_date,
                "followup_partners": set(),
            }
        group_analysis[g_name]["total"] += amt
        group_analysis[g_name]["fys"].add(fy)
        if bill_date and (
            not group_analysis[g_name]["first_date"]
            or bill_date < group_analysis[g_name]["first_date"]
        ):
    ...
    current_only_summary = dict(
        sorted(current_only_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    )
    prev_accum_summary = dict(
        sorted(prev_accum_summary.items(), key=lambda x: x[1]["total"], reverse=True)
    )
    ...
    current_fp_cat = dict(
        sorted(current_fp_cat.items(), key=lambda x: sum(x[1].values()), reverse=True)
    )
    prev_fp_cat = dict(
        sorted(prev_fp_cat.items(), key=lambda x: sum(x[1].values()), reverse=True)
    )
    prev_fp_period = dict(
        sorted(prev_fp_period.items(), key=lambda x: sum(x[1].values()), reverse=True)
    )
...
def periodic_group_detail_svg(group_name, rows):
    ...
    for label, _, col_width, _ in columns:
        parts.append(
            f'<rect x="{x}" y="{y}" width="{col_width}" height="{header_height}" fill="#34495e" stroke="#b7c4cc"/>'
        )
        parts.append(
            f'<text x="{x + 5}" y="{y + 19}" fill="#ffffff" font-family="Arial, sans-serif" font-size="11" font-weight="700">{html.escape(label)}</text>'
        )
        x += col_width
    ...
    if not rows:
        ...
        parts.append(
            f'<rect x="{padding}" y="{y_row}" width="{table_width}" height="{row_height}" fill="#ffffff" stroke="#cfd8dc"/>'
        )
        parts.append(
            f'<text x="{padding + 8}" y="{y_row + 18}" fill="#34495e" font-family="Arial, sans-serif" font-size="11">No records found.</text>'
        )
    for row_number, row in enumerate(rows or []):
        ...
        for _, key, col_width, char_limit in columns:
            ...
            parts.append(
                f'<rect x="{x}" y="{y_row}" width="{col_width}" height="{row_height}" fill="{fill}" stroke="#cfd8dc"/>'
            )
            parts.append(
                f'<text x="{x + 5}" y="{y_row + 17}" fill="#24384a" font-family="Arial, sans-serif" font-size="10">{html.escape(value)}</text>'
            )
            x += col_width
    ...
    for _, key, col_width, _ in columns:
        parts.append(
            f'<rect x="{x}" y="{total_y}" width="{col_width}" height="{total_height}" fill="#f1f4f6" stroke="#34495e"/>'
        )
        if key == "party_name":
            parts.append(
                f'<text x="{x + 5}" y="{total_y + 20}" fill="#1f2d3a" font-family="Arial, sans-serif" font-size="12" font-weight="700">Grand Total</text>'
            )
        elif key == "amount_display":
            parts.append(
                f'<text x="{x + 5}" y="{total_y + 20}" fill="#1f2d3a" font-family="Arial, sans-serif" font-size="12" font-weight="700">{html.escape("Rs " + format_indian_currency(total_amount, decimals=False))}</text>'
            )
        x += col_width
    parts.append("</svg>")
...
def periodic_group_detail_pdf(group_name, rows):
    ...
    for _, key, col_width, _ in columns:
        if key == "party_name":
            ...
            commands.append(
                f"BT {x + 3} {total_y + 8} Td ({pdf_escape('Rs ' + format_indian_currency(total_amount, decimals=False))}) Tj ET"
            )
        x += col_width
    commands.append("Q")
...
def build_report_export_data():
    ...
    periodic_ep_summary = dict(
        sorted(ep_analysis.items(), key=lambda x: x[1], reverse=True)
    )
    sorted_categories = sorted(list(all_categories))
...
def summary_export_rows(summary, name_label):
        ...
        export_rows.append(
            {
                ...
                "Percent": round((data.get("total", 0) / grand_total * 100), 2)
                if grand_total
                else 0,
            }
        )
    export_rows.append(
        {
            name_label: "Grand Total",
            "Total": grand_total,
            "Percent": 100 if grand_total else 0,
        }
    )
    return export_rows
...
def pivot_export_rows(pivot_data, categories, total_amount):
    ...
    for partner, cats in pivot_data.items():
        ...
        row["Percent"] = (
            round((partner_total / total_amount * 100), 2) if total_amount else 0
        )
...
def apply_excel_date_columns(
    writer, headers=("Bill Date", "Due Date", "First Bill Date", "Date")
):
...
def write_combined_summary_sheet(writer, data):
    ...
    sections = [
        (
            "Followup Summary",
            summary_export_rows(data["followup_summary"], "Followup Partner"),
        ),
    ...
    for title, rows in sections:
        sheet.cell(row=current_row, column=1, value=title).font = Font(
            bold=True, size=13
        )
        current_row += 1
        for row in dataframe_to_rows(pd.DataFrame(rows), index=False, header=True):
            for col_idx, value in enumerate(row, 1):
                ...
                if (
                    current_row == 2
                    or sheet.cell(row=current_row - 1, column=1).value == title
                ):
                    cell.font = Font(bold=True)
            current_row += 1
        current_row += 2
...
def update_report_row():
    ...
    if duplicate_ref:
        conn.close()
        flash(
            f"Unable to update row. Ref. No. {duplicate_ref.get('ref_no')} already exists."
        )
        return redirect(url_for("report"))
    ...
    if should_add_to_master or (
        matching_client and matching_master_group.strip() == client_group
    ):
        group_override = ""
...
def soft_delete_report_row():
    ...
    deleted_count = soft_delete_billing_rows(
        cursor, [row_id], request.form.get("delete_reason", "").strip()
    )
    conn.commit()
    conn.close()

    flash(
        "Report row moved to deleted records."
        if deleted_count
        else "Report row not found."
    )
    return redirect(url_for("report"))
...
def control_panel_bulk_delete_report():
    ...
    for row in rows:
        ...
        item = summary_lookup.setdefault(
            short_name, {"short_name": short_name, "bill_count": 0, "total_amount": 0}
        )
        item["bill_count"] += 1
        item["total_amount"] += float(row.get("amount") or 0)
...
def add_report_row():
    ...
    if not firm_name or not bill_date_obj or not ref_no or not party_name:
        flash(
            "Unable to add row. Please check firm, bill date, ref no., and Party's Name."
        )
        return redirect(url_for("report"))
    ...
    try:
        ...
        if duplicate_ref:
            flash(
                f"Unable to add row. Ref. No. {duplicate_ref.get('ref_no')} already exists."
            )
            return redirect(url_for("report"))
        ...
        inserted = insert_billing_row(
            cursor, firm_name, bill_date_obj, ref_no, party_name, amount
        )
        if not inserted:
            flash(
                "Unable to add row. Please check firm, bill date, ref no., Party's Name, and amount."
            )
            return redirect(url_for("report"))
        ...
        if "database is locked" in str(exc).lower():
            flash(
                "Report row could not be added because the database is busy. Please try again in a few seconds.",
                "error",
            )
        else:
...
def run_report_followup_logic():
    ...
    try:
        ...
        conn.commit()
        flash(
            f"Followup partner logic run successfully for {updated_count} open row(s)."
        )
    except Exception as exc:
        conn.rollback()
...
def import_followup_choice():
    ...
    try:
        if choice == "yes":
            updated_count = run_followup_partner_logic(cursor)
            flash(
                f"Followup partner rule run successfully for {updated_count} open row(s)."
            )
        else:
            updated_count = copy_existing_followup_for_import_batch(cursor, batch_id)
            flash(
                f"Existing group followup partner copied for {updated_count} imported row(s)."
            )
        conn.commit()
    except Exception as exc:
        conn.rollback()
...
def download_report_excel():
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(export_rows).to_excel(
            writer, index=False, sheet_name="Main Report"
        )
        ...
        for fy in data["all_fys"]:
            ...
            for ep in data["all_eps"]:
                amount = (
                    data["period_ep_summary"].get(ep, {}).get(fy, {"total": 0})["total"]
                )
                row_data[ep] = amount
                row_total += amount
        ...
        write_titled_sheet(
            writer, "EP Period Summary", f"Summary as on {as_on_label}", ep_period_rows
        )

        pd.DataFrame(
            periodic_group_export_rows(data["current_only_summary"], "Current Period")
        ).to_excel(writer, index=False, sheet_name="Current Period")
        ...
        writer.book["Current Period"]["A2"] = (
            f"Period: Current Period Only | Export as on {as_on_label}"
        )
        writer.book["Current Period"].freeze_panes = "A4"

        pd.DataFrame(
            periodic_group_export_rows(data["prev_accum_summary"], "Previous Years")
        ).to_excel(writer, index=False, sheet_name="Previous Years")
        ...
        writer.book["Previous Years"]["A2"] = (
            f"Period: Previous Years | Export as on {as_on_label}"
        )
        writer.book["Previous Years"].freeze_panes = "A4"

        total_current = sum(
            item["total"] for item in data["current_only_summary"].values()
        )
        total_previous = sum(
            item["total"] for item in data["prev_accum_summary"].values()
        )
        pd.DataFrame(
            pivot_export_rows(
                data["current_fp_cat"], data["all_categories"], total_current
            )
        ).to_excel(writer, index=False, sheet_name="Current FP Category")
        pd.DataFrame(
            pivot_export_rows(
                data["prev_fp_cat"], data["all_categories"], total_previous
            )
        ).to_excel(writer, index=False, sheet_name="Previous FP Category")

        periodic_ep_rows = summary_export_rows(
            {
                name: {"count": 0, "total": total}
                for name, total in data["periodic_ep_summary"].items()
            },
            "EP Name",
        )
        pd.DataFrame(periodic_ep_rows).to_excel(
            writer, index=False, sheet_name="Periodic EP Summary"
        )
...
def download_current_report_excel():
    ...
    data = build_report_export_data()
    rows_by_id = {
        int(row.get("id")): row for row in data["rows"] if row.get("id") is not None
    }
    ...
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(export_rows, columns=selected_columns).to_excel(
            writer, index=False, sheet_name="Current Report"
        )
...
def client_master_view():
    ...
    crp_options = sorted(
        {crp for crp_list in group_crp_map.values() for crp in crp_list}
    )
    conn.close()
    return render_template(
        "client_master.html",
...
def client_group_master_view():
    ...
    conn.close()
    return render_template(
        "client_group_master.html", groups=groups, crp_options=crp_options
    )
...
def client_group_clients_excel(group_id):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(export_rows).to_excel(
            writer, index=False, sheet_name="Group Clients"
        )
        apply_workbook_formatting(writer)
    output.seek(0)
    safe_group = (
        re.sub(r"[^A-Za-z0-9_-]+", "_", group["group_name"]).strip("_")
        or "client_group"
    )
    return send_file(
        output,
...
def download_client_group_master_excel():
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame(export_rows).to_excel(
            writer, index=False, sheet_name="Client Groups"
        )
        apply_workbook_formatting(writer)
...
def bulk_update_client_groups():
    ...
    missing_columns = [
        label for key, label in required_columns.items() if key not in column_map
    ]
    if missing_columns:
    ...
    try:
        ...
        cursor.execute(
            "SELECT crp_name FROM crp_master WHERE crp_name IS NOT NULL AND trim(crp_name) != ? ",
            ("",),
        )
        valid_crps = {
        ...
        for dataframe_index, row in dataframe.iterrows():
            ...
            if existing_group_name is None:
                raise ValueError(
                    f"Excel row {excel_row}: ID {group_id} does not exist."
                )
            ...
            crp_name = normalize_master_name(
                clean_excel_value(row[column_map["crpname"]])
            )
            if crp_name:
                canonical_crp = valid_crps.get(master_name_key(crp_name))
                if not canonical_crp:
                    raise ValueError(
                        f'Excel row {excel_row}: CRP Name "{crp_name}" is not in CRP Master.'
                    )
                crp_name = canonical_crp
            reffered_by = normalize_master_name(
                clean_excel_value(row[column_map["referredby"]])
            )
            ...
            updates.append(
                (crp_name, reffered_by, induction_date, group_id, existing_group_name)
            )
        ...
        conn.commit()
        flash(
            f"Bulk Update successful: {len(updates)} Client Group(s) updated.",
            "success",
        )
    except ValueError as exc:
        conn.rollback()
...
def add_client_group():
    ...
    induction_date_obj = (
        parse_input_date(induction_date_raw) if induction_date_raw else None
    )
    group_induction_date = (
        induction_date_obj.strftime("%Y-%m-%d") if induction_date_obj else ""
    )
        ...
        if existing_group:
            ...
            """,
                (
                    ...
                    group_induction_date
                    or existing_group.get("group_induction_date")
                    or "",
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    existing_group.get("id"),
                ),
...
def update_client_group():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT group_name FROM client_group_master WHERE id = ?", (group_id,)
    )
    ...
    try:
        duplicate_group = find_master_by_name(
            cursor, "client_group_master", "group_name", group_name, exclude_id=group_id
        )
        if duplicate_group:
            canonical_group = duplicate_group.get("group_name")
            update_normalized_references(
                cursor, "client_master", "client_group", old_group_name, canonical_group
            )
            update_normalized_references(
                cursor,
                "billing_report",
            ...
            """,
                (
                    ...
                    group_induction_date
                    or duplicate_group.get("group_induction_date")
                    or "",
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            conn.commit()
            flash(
                "Client group already existed. Records were merged into the existing group."
            )
            return redirect(url_for("client_group_master_view"))
        ...
        update_normalized_references(
            cursor, "client_master", "client_group", old_group_name, group_name
        )
        update_normalized_references(
            cursor, "billing_report", "group_override", old_group_name, group_name
        )
        update_normalized_references(
            cursor, "receipt_register", "client_group", old_group_name, group_name
        )
        update_normalized_references(
            cursor,
            "receipt_adjustment_register",
        ...
        if group:
            ...
            log_deleted_record(
                ...
                " | ".join(
                    part
                    for part in [
                        f"CRP: {group_dict.get('crp_name')}"
                        if group_dict.get("crp_name")
                        else "",
                        f"Referred By: {group_dict.get('reffered_by')}"
                        if group_dict.get("reffered_by")
                        else "",
                    ]
                    if part
                ),
...
def update_crp():
    ...
    try:
        duplicate_crp = find_master_by_name(
            cursor, "crp_master", "crp_name", crp_name, exclude_id=crp_id
        )
        if duplicate_crp:
            canonical_crp = duplicate_crp.get("crp_name")
            update_normalized_references(
                cursor, "client_group_master", "crp_name", old_crp_name, canonical_crp
            )
            update_normalized_references(
                cursor, "client_master", "crp_of_group", old_crp_name, canonical_crp
            )
            update_normalized_references(
                cursor, "receipt_register", "crp_of_group", old_crp_name, canonical_crp
            )
            update_normalized_references(
                cursor,
                "receipt_adjustment_register",
        ...
        update_normalized_references(
            cursor, "client_group_master", "crp_name", old_crp_name, crp_name
        )
        update_normalized_references(
            cursor, "client_master", "crp_of_group", old_crp_name, crp_name
        )
        update_normalized_references(
            cursor, "receipt_register", "crp_of_group", old_crp_name, crp_name
        )
        update_normalized_references(
            cursor,
            "receipt_adjustment_register",
...
def delete_crp():
    ...
    if crp_id:
        ...
        conn.close()
        flash(
            "CRP deleted from master. Existing group/client records were not deleted."
        )
    return redirect(url_for("crp_master_view"))
...
def update_firm():
    ...
    try:
        if target_firm:
            ...
            flash(
                f"Firm '{old_firm_name}' has been merged into '{firm_name}'. All bills updated."
            )
        else:
            # NORMAL RENAME
            cursor.execute(
    ...
    if firm_id:
        ...
        if firm:
            ...
            log_deleted_record(
                ...
                firm_dict,
                f"Short Name: {firm_dict.get('short_name')}"
                if firm_dict.get("short_name")
                else "",
            )
        cursor.execute("DELETE FROM firm_master WHERE id = ?", (firm_id,))
        conn.commit()
...
def control_panel_report_update():
    ...
    if (
        ...
        flash(
            "Please enter Report as on, Last Bill update, and Last Receipt update dates."
        )
...
def debtor_navbar_access():
    ...
    for user in users:
        ...
        user["access_keys"] = (
            set(DEBTOR_NAV_ACCESS_KEYS)
            if user["is_arif"]
            else saved_access.get(email, set())
        )
    return render_template(
        "navbar_access.html",
        users=users,
...
def update_debtor_navbar_access():
    ...
    if not users:
        flash(
            "No users found in the main app database. Access settings were not changed."
        )
        return redirect(url_for("debtor_navbar_access"))
...
def debtor_deleted_records():
    ...
    conn.close()
    return render_template(
        "deleted_records.html", deleted_rows=rows, active_page="control_panel"
    )
    ...
    if delete_log_id or legacy_billing_id:
        ...
        try:
            if delete_log_id:
                ...
                duplicate_ref = find_active_billing_ref(
                    cursor, deleted_bill["ref_no"], exclude_id=deleted_bill["id"]
                )
                if duplicate_ref:
                    raise ValueError(
                        f"Ref. No. {duplicate_ref.get('ref_no')} already exists in the main report. "
                ...
                flash(
                    "Deleted record recalled successfully."
                    if recalled
                    else "Deleted record not found."
                )
            conn.commit()
    ...
    try:
        ...
        if delete_log_id:
            ...
            if source_table == "billing_report" and source_pk:
                cursor.execute(
                    "DELETE FROM billing_report WHERE id = ? AND deleted_at IS NOT NULL",
                    (source_pk,),
                )
            cursor.execute(
                "DELETE FROM deleted_records_log WHERE id = ?", (delete_log_id,)
            )
        else:
            cursor.execute(
                "DELETE FROM billing_report WHERE id = ? AND deleted_at IS NOT NULL",
    ...
    if client_name:
        ...
        try:
            client_group, crp_of_group, reffered_by = resolve_client_group_parent(
                cursor, client_group, crp_of_group, reffered_by
            )
            existing_client = find_master_by_name(
                cursor, "client_master", "client_name", client_name
            )
            if existing_client:
                cursor.execute(
                    """
            ...
            cursor.execute(
                "INSERT INTO client_master (client_name, phone, email, gstin, client_group, crp_of_group, reffered_by, whatapp_group, client_category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    client_name,
                    phone,
                    email,
            ...
            if "database is locked" in str(exc).lower():
                flash(
                    "Client could not be added because the database is busy. Please try again in a few seconds.",
                    "error",
                )
            else:
...
def update_client():
    ...
    duplicate_client = find_master_by_name(
        cursor, "client_master", "client_name", client_name, exclude_id=client_id
    )
    if duplicate_client:
        cursor.execute(
            """
...
def upload_file():
    ...
    if file and file.filename.lower().endswith((".csv", ".xlsx", ".xls")):
        ...
        if duplicate_ref_count:
            ...
            more_text = (
                "..." if duplicate_ref_count > len(duplicate_ref_samples) else ""
            )
        ...
        if missing_client_count:
            message_parts.append(
                f"{missing_client_count} imported party name(s) are not in Client Master. Check Missing Clients Report for current Main Report status."
            )
        else:
        ...
        if batch_id:
            ...
            return_to = (
                f"{return_to}{separator}import_batch={batch_id}{followup_prompt}"
            )
    else:
...
def bulk_import_preview():
    ...
    for row in rows:
        ...
        group = source_groups.setdefault(
            source_name, {"name": source_name, "count": 0, "total": 0}
        )
        group["count"] += 1
...
def bulk_import_confirm():
    ...
    # DATABASE CHECKPOINT BEFORE BULK IMPORT
    try:
        create_database_checkpoint(
            note=(f"Before Bulk Import: {token_meta.get('filename') or stored_name}")
        )
    except Exception as exc:
        flash(f"Bulk Import aborted. Database checkpoint failed: {exc}", "error")
        return redirect(url_for("report"))
    ...
    try:
        for row, firm_name in zip(rows, canonical_firms):
            if insert_billing_row(
                cursor,
                firm_name,
                parse_input_date(row["bill_date"]),
                row["ref_no"],
                row["party_name"],
                row["amount"],
                parse_input_date(row.get("due_date") or ""),
                import_batch_id=batch_id,
            ):
    ...
    if duplicate_refs:
        ...
        message += (
            f" {len(duplicate_refs)} duplicate Ref. No. row(s) skipped: {samples}"
        )
        if len(duplicate_refs) > 10:
            message += "..."
    if missing_count:
        message += (
            f" {missing_count} imported party name(s) are missing in Client Master."
        )
    ...
    if file and file.filename.endswith(".xlsx"):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                client_name = get_col_val(
                    row, ["Client Name", "Name", "Party Name", "Party", "Client"]
                )
                ...
                reffered_by = get_col_val(
                    row, ["Reffered By", "Referred By", "RefferedBy", "ReferredBy"]
                )
                whatapp_group = get_col_val(
                    row,
                    ["Whatapp Group", "Whatapp", "Whatsapp Group", "WhatsApp Group"],
                )
                    ...
                    if duplicate_client:
                        ...
                        """,
                            (
                                ...
                                gstin or duplicate_client.get("gstin") or "",
                                client_group
                                or duplicate_client.get("client_group")
                                or "",
                                crp_of_group
                                or duplicate_client.get("crp_of_group")
                                or "",
                                reffered_by
                                or duplicate_client.get("reffered_by")
                                or "",
                                whatapp_group
                                or duplicate_client.get("whatapp_group")
                                or "",
                                client_category
                                or duplicate_client.get("client_category")
                                or "",
                                duplicate_client.get("id"),
                            ),
                        )
                        cursor.execute(
                            "DELETE FROM client_master WHERE id=?", (client_id,)
                        )
                        imported_count += 1
                        continue
                    ...
                    if existing_client:
                        ...
                        """,
                            (
                                ...
                                gstin or existing_client.get("gstin") or "",
                                client_group
                                or existing_client.get("client_group")
                                or "",
                                crp_of_group
                                or existing_client.get("crp_of_group")
                                or "",
                                reffered_by or existing_client.get("reffered_by") or "",
                                whatapp_group
                                or existing_client.get("whatapp_group")
                                or "",
                                client_category
                                or existing_client.get("client_category")
                                or "",
                                existing_client.get("id"),
                            ),
                        )
            ...
            conn.commit()
            flash(
                f"{imported_count} clients processed (updated/added) successfully!",
                "success",
            )
        except Exception as e:
...
def clear_report():
    ...
    for row in cursor.fetchall():
        ...
        log_deleted_record(
            ...
            row_dict.get("id"),
            "Billing Report",
            row_dict.get("party_name")
            or row_dict.get("ref_no")
            or f"Bill #{row_dict.get('id')}",
            row_dict,
            " | ".join(
                part
                for part in [
