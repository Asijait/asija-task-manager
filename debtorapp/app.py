...
def normalize_reconciliation_value(value):
    if value is None:
        return ""
...
def normalize_reconciliation_ref(value):
    if value is None:
        return ""
    ...
    # Ref numbers should be compared independent of:
    # spaces, -, /, _, punctuation and case.
    return re.sub(r"[^a-z0-9]", "", value.casefold())


def extract_receipt_bill_rows(receipt_file):
    """Extract bill allocations from Tally's receipt-register block layout."""
    try:
        workbook = pd.ExcelFile(receipt_file)
        receipt_df = pd.read_excel(workbook, sheet_name=0, header=None)
    ...
    for values in receipt_df.itertuples(index=False, name=None):
        ...
        voucher_type = normalize_reconciliation_ref(row[6])
        particulars = normalize_reconciliation_ref(row[1])
        ...
        if particulars in {"agst ref", "new ref"} and normalize_reconciliation_ref(
            row[2]
        ):
            ref_no = str(row[2]).strip()
            try:
                amount = float(row[4])
            except (TypeError, ValueError):
                continue
            parsed_receipt_date = parse_excel_date(current_receipt.get("receipt_date"))
            receipt_rows.append(
                {
                    "Receipt Date": parsed_receipt_date.strftime("%Y-%m-%d")
                    if parsed_receipt_date
                    else "",
                    "Firm": str(current_receipt.get("firm") or "").strip(),
                    "Party's Name": str(
                        current_receipt.get("party_name") or ""
                    ).strip(),
                    "Ref. No.": ref_no,
...
def build_debtor_reconciliation_workbook(uploaded_file, receipt_file=None):
    ...
    for _, source_row in source_df.iterrows():
        party_key = normalize_reconciliation_value(source_row.get("Party Name"))

        ref_key = normalize_reconciliation_ref(source_row.get("Ref No."))

        if party_key:
            source_by_party.setdefault(party_key, []).append(source_row)
    ...
    for sheet_row_number, app_row in enumerate(app_rows, start=2):
        app_party = app_row["party_name"] or ""
        app_ref = app_row["ref_no"] or ""

        party_key = normalize_reconciliation_value(app_party)

        ref_key = normalize_reconciliation_ref(app_ref)
        ...
        party_matches = source_by_party.get(party_key, [])
        ...
        if party_matches:
            ...
            if len(party_matches) == 1:
                source_row = party_matches[0]
                matched_by = "Party Name"

            # --------------------------------------------------
            # DUPLICATE PARTY
            #
            # Use Ref No ONLY to identify the correct row.
            # --------------------------------------------------

            else:
                for candidate in party_matches:
                    candidate_ref = normalize_reconciliation_ref(
                        candidate.get("Ref No.")
                    )

                    if ref_key and candidate_ref == ref_key:
                        source_row = candidate
                        matched_by = "Party Name + Ref No."
                        break

                # --------------------------------------------------
                # If Ref doesn't identify the row, use exact amount.
                # --------------------------------------------------
                    ...
                    for candidate in party_matches:
                        try:
                            candidate_amount = float(
                                candidate.get("Pending Amount") or 0
                            )
                        except (TypeError, ValueError):
                            continue

                        if abs(candidate_amount - current_amount) <= 0.005:
                            source_row = candidate
                            matched_by = "Party Name + Amount"
                            break

                # --------------------------------------------------
                # Party exists.
                # NEVER DELETE because of duplicate ambiguity.
                # --------------------------------------------------
        ...
        # ======================================================
        # 2. PARTY NAME NOT FOUND
        #
        # NOW CHECK REF NO.
        # ======================================================

        if source_row is None and ref_key:
            ref_matches = source_by_ref.get(ref_key, [])

            if ref_matches:
                source_row = ref_matches[0]
                matched_by = "Ref No."
            ...
            delete_rows.append(
                {
                    **base,
                    ...
                    "Reason": ("Party Name and Ref No. not found in source report"),
                }
            )
        ...
        try:
            correct_amount = float(source_row.get("Pending Amount") or 0)
        except (TypeError, ValueError):
            continue
    ...
    if receipt_file is not None and getattr(receipt_file, "filename", None):
        receipt_entries = extract_receipt_bill_rows(receipt_file)

        deleted_party_names = {
            normalize_reconciliation_value(row.get("Party's Name"))
            for row in delete_rows
        }

        deleted_refs = {
            normalize_reconciliation_ref(row.get("Ref. No.")) for row in delete_rows
        }

        for receipt in receipt_entries:
            receipt_party = normalize_reconciliation_value(receipt.get("Party's Name"))

            receipt_ref = normalize_reconciliation_ref(receipt.get("Ref. No."))

            if receipt_party and receipt_party in deleted_party_names:
                receipt_rows.append(receipt)
                continue

            if receipt_ref and receipt_ref in deleted_refs:
                receipt_rows.append(receipt)
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        pd.DataFrame(
            [
                [
                    "Total records in debtor report",
                    None,
                    None,
                    len(app_rows),
                ],
                ...
                [
                    "Records to UPDATE (Amount mismatch)",
                    None,
                    None,
                    len(update_rows),
                ],
        ...
        summary_sheet["B3"] = "Party Name primary; Ref No. fallback"

        summary_sheet["B4"] = (
            "Ref No. is used only when Party Name is absent or duplicated."
        )
        ...
        writer.book["Delete"]["A1"] = "Records to DELETE"

        writer.book["Delete"]["A2"] = (
            "Deleted only when Party Name AND Ref No. are not found."
        )
        ...
        pd.DataFrame(
            ...
            writer,
            index=False,
            sheet_name="Update Record (Amount)",
            startrow=3,
        )

        writer.book["Update Record (Amount)"]["A1"] = "Records to UPDATE (Amount)"
        ...
        pd.DataFrame(
            ...
            writer,
            index=False,
            sheet_name="Receipts to Post",
            startrow=3,
        )

        writer.book["Receipts to Post"]["A1"] = "Receipts to Post"
