...
PAGE_PERMISSIONS = [
    ...
    {
        "key": "daily_debtor_report",
        "label": "Debtor Report",
        "path": "/daily-debtor-report",
    },
    ...
    {
        "key": "assign_to_master",
        "label": "Department Name Master",
        "path": "/assign-to-master",
    },
    {
        "key": "user_power_management",
        "label": "User Powers",
...
def set_debtor_report_update_dates(
    report_as_on_date, last_bill_update_date, last_receipt_update_date
):
    conn = debtor_db.connect(DEBTOR_REPORT_DATABASE)
    cursor = conn.cursor()
    cursor.execute("""
...
def safe_restore_database(project_key, backup_id):
    ...
    if project_key == "main_app" and main_db.use_postgres(DATABASE):
        raise ValueError(
            "Main app is using PostgreSQL. Restore it with psql from a PostgreSQL backup file."
        )
    ...
    if not project_entry or project_entry.get("status") != "backed_up":
        raise FileNotFoundError(
            "Selected project database is not available in this backup."
        )

    backup_file = project_entry.get("backup_file")
    backup_path = os.path.abspath(os.path.join(backup_folder, backup_file))
    if not backup_path.startswith(backup_folder + os.sep) or not os.path.exists(
        backup_path
    ):
    ...
    safety_folder = os.path.join(
        DATABASE_BACKUP_DIR, f"before_restore_{project_key}_{safety_id}"
    )
...
def init_db():
    ...
    try:
        ...
        for user_row in c.fetchall():
            if not str(user_row["alias_name"] or "").strip():
                ...
                c.execute(
                    "UPDATE users SET alias_name = ? WHERE id = ?",
                    (default_alias, user_row["id"]),
                )
        conn.commit()
    ...
    try:
        ...
        if "access_level" not in role_columns:
            c.execute(
                "ALTER TABLE user_roles ADD COLUMN access_level TEXT DEFAULT 'edit'"
            )
            c.execute(
                "UPDATE user_roles SET access_level = 'edit' WHERE access_level IS NULL"
            )
        conn.commit()
    except Exception as e:
    ...
    # A signup is held here until the owner of the email address verifies it.
    # No user account or page permissions are created before that verification.
    c.execute("""
    ...
    for user_id, email in c.fetchall():
        ...
        if str(email).lower() in ADMIN_EMAILS:
            for permission_key in [p["key"] for p in PAGE_PERMISSIONS] + [
                p["key"] for p in SPECIAL_PERMISSIONS
            ]:
                c.execute(
    ...
    unassigned_users = c.execute(
        ...
        WHERE lower(u.email) LIKE '%@asija.in'
          ...
          AND NOT EXISTS (
              ...
              WHERE wcr.requester_user_id = u.id
                AND wcr.action = 'user_access'
    ...
    for user in unassigned_users:
        c.execute(
            """
            INSERT INTO work_change_requests (requester_user_id, action, work_id, payload)
            VALUES (?, 'user_access', NULL, ?)
            """,
            (user["id"], json.dumps({"user_id": user["id"], "email": user["email"]})),
        )
...
def get_work_report_allottees():
    ...
    try:
        rows = conn.execute(
            "SELECT email, alias_name FROM users ORDER BY email ASC"
        ).fetchall()
        result = []
        seen = set()
        for row in rows:
...
def get_work_report_current_user():
    ...
    try:
        row = conn.execute(
            "SELECT email, alias_name FROM users WHERE id = ?", (user_id,)
        ).fetchone()
        if not row:
            return None
        email = str(row["email"] or "").strip()
        ...
        for visible in visible_rows:
            ...
            visible_name = (
                str(visible["alias_name"] or "").strip()
                or visible_email.split("@", 1)[0]
            )
            if visible_name and visible_name.lower() not in {
                item.lower() for item in visible_names
            }:
...
def create_restore_point(label, reason="manual"):
    ...
    for root, dirs, filenames in os.walk(BASE_DIR):
        ...
        if rel_root == ".":
            rel_root = ""
        dirs[:] = [
            d for d in dirs if not should_skip_restore_path(os.path.join(rel_root, d))
        ]
...
def restore_project_from_point(filename):
    ...
    if not archive_path.startswith(restore_dir_abs + os.sep) or not os.path.isfile(
        archive_path
    ):
        raise ValueError("Restore point not found")

    safety_point = create_restore_point(
        f"Before restoring {filename}", reason="before_restore"
    )
    ...
    with zipfile.ZipFile(archive_path, "r") as zf:
        for member in zf.infolist():
            ...
            if rel_path == "restore_manifest.json" or should_skip_restore_path(
                rel_path
            ):
                continue
...
def ensure_current_user_can_edit_work(conn, work_id, payload=None):
    ...
    if payload and not can_current_user_edit_assignee(conn, payload.get("assignee_id")):
        raise PermissionError(
            "You cannot move this task to a department where you do not have edit access."
        )
...
def is_date_scheduled(start_dt, target_dt, frequency):
    ...
    is_day_match = (
        is_now_month_end if was_month_end else (target_dt.day == start_dt.day)
    )
    ...
    if frequency == "quarterly":
        months_diff = (target_dt.year - start_dt.year) * 12 + (
            target_dt.month - start_dt.month
        )
...
def valid_asija_email(email):
    return bool(re.fullmatch(r"[A-Za-z0-9._%+-]+@asija\.in", email, re.IGNORECASE))


def send_signup_email(recipient_email, subject, html_content, text_content):
    """Use the configured email service without making it a startup dependency."""
...
def queue_signup_email(recipient_email, subject, html_content, text_content):
    ...
    def deliver():
        try:
            ...
            app.logger.exception("Unable to send signup email to %s", recipient_email)
...
def signup():
    ...
    email = normalize_signup_email(request.form.get("email"))
    password = request.form.get("password", "")
    confirm_password = request.form.get("confirm_password", "")

    if not valid_asija_email(email):
        return render_template(
            "signup.html",
            error="Please use a valid @asija.in email address.",
            email=email,
        )
    if action == "send_otp":
        if len(password) < 8:
        ...
        if password != confirm_password:
            return render_template(
                "signup.html", error="Passwords do not match.", email=email
            )
    ...
    if action == "send_otp":
        ...
        expiry = OTPService.get_expiry()
        conn = get_db_connection()
        try:
            conn.execute(
                """
                INSERT INTO signup_otps (email, password_hash, otp_hash, expires_at, last_sent_at)
                VALUES (?, ?, ?, ?, ?)
        ...
        queue_signup_email(
            email,
            "Your Asija Workspace verification code",
            (
                "<p>Your verification code is <strong>"
                f"{otp}</strong>.</p><p>It expires in "
                f"{OTPService.OTP_EXPIRY_MINUTES} minutes.</p>"
            ),
            f"Your verification code is {otp}. It expires in {OTPService.OTP_EXPIRY_MINUTES} minutes.",
        )
        return render_template(
            "signup.html",
            success="OTP is being sent. Enter it below to verify your email address.",
            email=email,
            otp_sent=True,
        )
    ...
    try:
        pending = conn.execute(
            "SELECT email, password_hash, otp_hash, expires_at FROM signup_otps WHERE email = ?",
            (email,),
        ).fetchone()
        if not pending:
            return render_template(
                "signup.html", error="Send an OTP before verifying.", email=email
            )
        try:
        ...
        alias_name = email.split("@", 1)[0]
        conn.execute(
            "INSERT INTO users (email, password, alias_name) VALUES (?, ?, ?)",
            (email, pending["password_hash"], alias_name),
        )
        ...
        conn.execute(
            ...
            (
                new_user["id"],
                json.dumps({"user_id": new_user["id"], "email": email}),
            ),
        )
        conn.execute("DELETE FROM signup_otps WHERE email = ?", (email,))
        conn.commit()
        ...
        conn.rollback()
        app.logger.exception("Unable to verify signup for %s", email)
        return render_template(
            "signup.html", error=f"Unable to complete signup: {exc}", email=email
        )
    finally:
        conn.close()

    admin_link = url_for("user_role_management", _external=True)
    notification = (
        f"<p><strong>{email}</strong> has verified their email address and is waiting for page access.</p>"
        f'<p><a href="{admin_link}">Open User Powers</a></p>'
    )
    for admin_email in {"arif.siddiqui@asija.in"}:
        if admin_email == email:
            continue
        queue_signup_email(
            admin_email,
            "New verified Asija Workspace user awaiting access",
            notification,
            f"{email} has verified their email address and is waiting for page access. Open User Powers: {admin_link}",
        )

    return render_template(
        "signup.html",
        success=(
            "Email verified. Your account is ready, but no pages are available until an "
            "administrator assigns access in User Powers."
        ),
        verified=True,
        email=email,
    )
...
def update_debtor_report_dates():
    ...
    set_debtor_report_update_dates(
        report_as_on_date, last_bill_update_date, last_receipt_update_date
    )
    return redirect(url_for("control_panel"))
...
def get_users_list():
    ...
    conn = get_db_connection()
    users = conn.execute(
        "SELECT id, email, alias_name FROM users ORDER BY email ASC"
    ).fetchall()
    roles = conn.execute(
        "SELECT user_id, assignee_id, access_level FROM user_roles"
    ).fetchall()
...
def create_user_account():
    ...
    if email_prefix.lower() not in alias_name.lower():
        return jsonify(
            {"success": False, "error": f"Alias must contain '{email_prefix}'"}
        ), 400
...
def update_user_account(user_id):
    ...
    try:
        ...
        if password:
            ...
            conn.execute(
                "UPDATE users SET email = ?, alias_name = ? WHERE id = ?",
                (email, alias_name, user_id),
            )
...
def bulk_delete_user_accounts():
    ...
    try:
        ...
        if len(users) != len(user_ids):
            return jsonify(
                {"success": False, "error": "One or more users no longer exist"}
            ), 400
        protected = [
            user["email"]
            for user in users
        ...
        if protected:
            return jsonify(
                {"success": False, "error": "Admin/current user cannot be bulk deleted"}
            ), 400

        for user_id in user_ids:
            conn.execute(
                "UPDATE work_master SET assigned_user_id = NULL WHERE assigned_user_id = ?",
                (user_id,),
            )
            conn.execute("DELETE FROM user_roles WHERE user_id = ?", (user_id,))
            conn.execute("DELETE FROM user_permissions WHERE user_id = ?", (user_id,))
            conn.execute(
                "DELETE FROM work_report_visibility WHERE viewer_user_id = ? OR visible_user_id = ?",
                (user_id, user_id),
            )
...
def delete_user_account(user_id):
    if session.get("user_id") == user_id:
        return jsonify(
            {"success": False, "error": "You cannot delete your own login"}
        ), 400
    ...
    try:
        ...
        if user["email"].lower() == "arif.siddiqui@asija.in":
            return jsonify(
                {"success": False, "error": "Arif user cannot be deleted"}
            ), 400
...
def links_and_others():
    ...
    try:
        ...
        db_links = conn.execute(
            "SELECT * FROM important_links ORDER BY category"
        ).fetchall()
        categories_list = [
            row["category"]
            for row in conn.execute(
                "SELECT DISTINCT category FROM important_links ORDER BY category"
            ).fetchall()
        ]
        ...
        for item in data:
            conn.execute(
                ...
                (
                    ...
                    item["tDate"],
                    item["status"],
                    item.get("allocatedTo", ""),
                    item.get("totalSeconds", 0),
                ),
            )
        conn.commit()
...
def links_add_link():
    ...
    conn.execute(
        ...
        (
            ...
            data.get("link_name"),
            data.get("url"),
            1 if data.get("is_special") else 0,
        ),
    )
    conn.commit()
    conn.close()
...
def api_restore_database_backup():
    ...
    if confirm != "RESTORE":
        return jsonify(
            {"success": False, "error": "Restore confirmation is required."}
        ), 400
    try:
...
def api_create_restore_point():
    ...
    try:
        point = create_restore_point(
            data.get("label", "Manual checkpoint"), reason="manual"
        )
...
def delete_work(work_id):
    ...
    try:
        if not has_user_permission("approve_requests"):
            ...
            work = conn.execute(
                "SELECT work_name FROM work_master WHERE id=? AND deleted_at IS NULL",
                (work_id,),
            ).fetchone()
            if not work:
                conn.close()
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                item["payload"] = {}
            item["can_approve"] = (
                has_user_permission("approve_requests")
                and item["status"] == "pending"
                and item["action"] != "user_access"
            )
            result.append(item)
...
def approve_work_change_request(request_id):
    ...
    try:
        row = conn.execute(
            "SELECT * FROM work_change_requests WHERE id = ?", (request_id,)
        ).fetchone()
        if not row:
...
def reject_work_change_request(request_id):
    ...
    try:
        row = conn.execute(
            "SELECT status FROM work_change_requests WHERE id = ?", (request_id,)
        ).fetchone()
        if not row:
            return jsonify({"success": False, "error": "Request not found"}), 404
        if row["status"] != "pending":
...
def restore_deleted_work_report(work_id):
    ...
    if not result.rowcount:
        return jsonify(
            {"success": False, "error": "Deleted Work Report record not found"}
        ), 404
    return jsonify({"success": True})
...
def get_assignees():
    ...
    if can_manage_users_or_departments():
        assignees = conn.execute(
            "SELECT * FROM assignee_master ORDER BY name ASC"
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT assignee_id FROM user_roles WHERE user_id = ? AND access_level IN ('view', 'edit')",
...
def add_assignee():
    ...
    try:
        conn.execute(
            "INSERT INTO assignee_master (name, email) VALUES (?, ?)", (name, email)
        )
        conn.commit()
        conn.close()
...
def update_assignee(a_id):
    ...
    try:
        conn.execute(
            "UPDATE assignee_master SET name = ?, email = ? WHERE id = ?",
            (name, email, a_id),
        )
        conn.commit()
        conn.close()
...
def get_user_roles():
    ...
    permissions = conn.execute(
        "SELECT user_id, permission_key FROM user_permissions"
    ).fetchall()
    visibility = conn.execute(
        "SELECT viewer_user_id, visible_user_id FROM work_report_visibility"
    ).fetchall()
    conn.close()
    ...
    for item in visibility:
        visibility_dict.setdefault(item["viewer_user_id"], []).append(
            item["visible_user_id"]
        )
        ...
        [
            {
                ...
                "assigned_ids": [
                    role["assignee_id"] for role in roles_dict.get(u["id"], [])
                ],
...
def save_user_roles():
    ...
    try:
        ...
        conn.execute(
            "DELETE FROM work_report_visibility WHERE viewer_user_id = ?", (user_id,)
        )
        valid_user_ids = {
        ...
        conn.execute(
            ...
            UPDATE work_change_requests
            SET status = 'approved', reviewer_user_id = ?, reviewed_at = CURRENT_TIMESTAMP
            WHERE requester_user_id = ? AND action = 'user_access' AND status = 'pending'
            """,
...
def get_assigned_work(date):
    ...
    try:
        ...
        if selected_dt < PROJECT_PERIOD_START or selected_dt > PROJECT_PERIOD_END:
            return jsonify(
                {"error": "Selected date is outside the project period"}
            ), 400
        ...
        def append_work_instance(work, occurrence_dt, only_unclosed=False):
            ...
            eff_assignee_id = (
                ...
                if (assigned and assigned["assignee_id"])
                else work["assignee_id"]
            )
            ...
            result.append(
                {
                    ...
                    "assignee_name": assigned["override_name"]
                    if (assigned and assigned["override_name"])
                    else work["assignee_name"],
                    "assignee_email": assigned["override_email"]
                    if (assigned and assigned["override_email"])
                    else work["assignee_email"],
        ...
        for work in all_works:
            schedule_date = (
                (work["work_start_date"] or work["work_date"])
                if date_type == "start"
                else work["work_date"]
            )
            ...
            try:
                ...
                print(
                    f"Skipping task {work['id']} due to invalid date format: {schedule_date}"
                )
                continue
...
def get_import_template():
    ...
    assignees = [
        ...
        for a in conn.execute(
            "SELECT name FROM assignee_master ORDER BY name ASC"
        ).fetchall()
    ]
    conn.close()
    ...
    dv_freq = DataValidation(
        type="list",
        formula1=f"'Fixed Data'!$A$2:$A${len(frequencies) + 1}",
        allow_blank=True,
    )
    ...
    dv_prio = DataValidation(
        type="list",
        formula1=f"'Fixed Data'!$B$2:$B${len(priorities) + 1}",
        allow_blank=True,
    )
    ...
    if assignees:
        dv_asgn = DataValidation(
            type="list",
            formula1=f"'Fixed Data'!$C$2:$C${len(assignees) + 1}",
            allow_blank=True,
        )
...
def validate_import():
    ...
    try:
        ...
        if "Input Data" not in wb.sheetnames:
            return jsonify(
                {"error": 'Invalid template: Missing "Input Data" sheet'}
            ), 400
        ws = wb["Input Data"]
    except Exception as e:
    ...
    assignee_map = {
        a["name"]: a["id"]
        for a in conn.execute("SELECT id, name FROM assignee_master").fetchall()
    }
    conn.close()
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if isinstance(raw_date, datetime):
            ...
            try:
                ...
                try:
                    ...
                    start_date = (
                        date_str  # Keep original value to show in error message
                    )
        ...
        if freq not in valid_frequencies:
            errors.append(
                f"Invalid Frequency (Must be: {', '.join(valid_frequencies)})"
            )
        ...
        try:
            ...
            if target_dt < PROJECT_PERIOD_START or target_dt > PROJECT_PERIOD_END:
                errors.append(
                    "Target Date outside project period (01.04.2026 - 31.03.2027)"
                )
        except ValueError:
        ...
        if work_start_date:
            try:
                ...
                if (
                    work_start_dt < PROJECT_PERIOD_START
                    or work_start_dt > PROJECT_PERIOD_END
                ):
                    errors.append(
                        "Work Start Date outside project period (01.04.2026 - 31.03.2027)"
                    )
            except ValueError:
                errors.append(
                    f"Invalid Work Start Date: {work_start_date} (Use DD.MM.YYYY)"
                )
            ...
            for work in all_works:
                ...
                if is_date_scheduled(task_start_dt, curr_dt, work["work_tat"]):
                    ...
                    current_assignee_name = (
                        assigned["override_name"]
                        if (assigned and assigned["override_name"])
                        else work["assignee_name"]
                    )
                    current_assignee_email = (
                        assigned["override_email"]
                        if (assigned and assigned["override_email"])
                        else work["assignee_email"]
                    )
                        ...
                        report_data.append(
                            {
                                ...
                                "status": current_status,
                                "actual_start": assigned["actual_start"]
                                if assigned
                                else "",
                                "actual_end": assigned["actual_end"]
                                if assigned
                                else "",
...
def assign_work():
    ...
    try:
        work = conn.execute(
            "SELECT id FROM work_master WHERE id = ? AND deleted_at IS NULL", (work_id,)
        ).fetchone()
        if not work:
            return jsonify(
                {"success": False, "error": "Task not found or already deleted"}
        ...
        existing = conn.execute(
            "SELECT id FROM work_assigned WHERE work_id = ? AND assigned_date = ?",
            (work_id, assigned_date),
        ).fetchone()

        if existing:
            conn.execute(
                """UPDATE work_assigned SET status=?, actual_start=?, actual_end=?, remarks=?, assignee_id=? WHERE id=?""",
                (
                    status,
                    actual_start,
                    actual_end,
            ...
            conn.execute(
                """INSERT INTO work_assigned (work_id, assigned_date, status, actual_start, actual_end, remarks, assignee_id) 
                           VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (
                    work_id,
                    assigned_date,
    ...
    for r_idx, row in enumerate(rows, data_start_row):
        for c_idx, value in enumerate(row, 1):
            ...
            cell.alignment = Alignment(
                horizontal="left" if c_idx == 1 or c_idx == 2 else "center",
                vertical="center",
            )
...
def update_status(assignment_id):
    ...
    return jsonify(
        {"success": True, "message": "Status tracking not needed in new flow"}
    ), 200
...
def remove_assignment(assignment_id):
    """This endpoint is kept for compatibility but not used in new flow"""
    return jsonify(
        {"success": True, "message": "No need to remove auto-assigned tasks"}
    ), 200
...
if __name__ == "__main__":
    try:
        ...
        app.run(
            host="0.0.0.0", debug=False, use_reloader=False, threaded=True, port=5001
        )
